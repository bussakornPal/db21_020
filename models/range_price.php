<?php class range_price{
    ...
    public static function search($key)
    {
        $range_priceList = [];
        require("connect_database.php");
        $sql="SELECT * FROM range_price NATURAL JOIN product where id_range like '%$key%' or min_range like '%$key%' or max_range like '%$key%' or price like '%$key%' or color_price like '%$key%' or id _product like '%$key%' ";
        $result=$conn->query($sql);
        while($my_row=$result->fetch_assoc())
        {
            $id_range = $my_row[id_range];
