<?php class detail_order{
    ...
    public static function getAll()
    {
        ...
        $sql = "SELECT id_doc,id_order_cus,id_cop,product.name_product,color.name_color,print_color,amount_order_cus FROM detail_order_customer NATURAL JOIN color_of_product NATURAL JOIN product NATURAL JOIN color ORDER BY id_order_cus";
    ...
    public static function search($key)
    {
        ...
        $sql = "SELECT id_doc,id_order_cus,id_cop,product.name_product,color.name_color,print_color,amount_order_cus FROM detail_order_customer NATURAL JOIN color_of_product NATURAL JOIN product NATURAL JOIN color
         WHERE (id_doc like'%$key%' or amount_order_cus like'%$key%' or id_order_cus like'%$key%' or id_cop like'%$key%' or name_color like '%$key%' or name_product like '%$key%') ORDER BY id_order_cus";
        $result=$conn->query($sql);
        while($my_row=$result->fetch_assoc())
        {
            $id_doc = $my_row[id_doc];
